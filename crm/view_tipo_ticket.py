...
@login_required
@secure_module
def tipoTicketView(request):
    data = {'titulo': 'Tipo Ticket de Atención',
            'modulo': 'CRM',
            'ruta': request.path,
            'fecha': str(date.today())
            }
    model = TipoTicketAtencion
    Formulario = TipoTicketAtencionForm
    ...
    if request.method == 'POST':
        ...
        try:
            with transaction.atomic():
                if action == 'add':
                    ...
                    if form.is_valid():
                        form.save()
                        log(f"Registro un tipo ticket {form.instance.__str__()}", request, "add", obj=form.instance.id)
                    ...
                    if form.is_valid() and filtro:
                        form.save()
                        log(f"Edito un tipo ticket  {form.instance.__str__()}", request, "change", obj=form.instance.id)
                        res_json.append({'error': False, "reload": True})
                    else:
                        raise FormError(form)
                    ...
                    filtro.status = False
                    filtro.save(request)
                    log(f"Elimino un tipo ticket {filtro.__str__()}", request, "del", obj=filtro.id)
        ...
        if 'action' in request.GET:
            ...
            if action == 'add':
                try:
                    ...
                    template = get_template("crm/tipo_ticket/form.html")
                ...
                return render(request, 'crm/tipo_ticket/form.html', data)
        ...
        return render(request, 'crm/tipo_ticket/listado.html', data)
