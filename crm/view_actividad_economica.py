...
from .forms import IndustriaForm, ActividadEconomicaForm
from .models import Industria, ActividadEconomica
from django.contrib import messages


@login_required
@secure_module
def actividadEconomicaView(request):
    data = {'titulo': 'Actividad Economica',
            'modulo': 'CRM',
            'ruta': request.path,
            'fecha': str(date.today())
            }
    model = ActividadEconomica
    Formulario = ActividadEconomicaForm
        ...
        try:
            with transaction.atomic():
                if action == 'add':
                    ...
                    if form.is_valid():
                        form.save()
                        log(f"Registro una actividad economica {form.instance.__str__()}", request, "add", obj=form.instance.id)
                    ...
                    if form.is_valid() and filtro:
                        form.save()
                        log(f"Edito una actividad economica  {form.instance.__str__()}", request, "change", obj=form.instance.id)
                        res_json.append({'error': False, "reload": True})
                    else:
                        raise FormError(form)
                    ...
                    filtro.status = False
                    filtro.save(request)
                    log(f"Elimino una actividad economica {filtro.__str__()}", request, "del", obj=filtro.id)
