...
@registrar_funcion(
    codigo='registrar_inscripcion_buceo',
    descripcion=('Guarda/actualiza el Cliente en el CRM (por cédula) con los datos '
                 'de la inscripción al curso de buceo. Best-effort: intenta además '
                 'el POST a SAGEST si el nodo tiene endpoint + path_inscripcion, '
                 'pero el éxito depende solo del guardado local.'),
    parametros={
    ...
    ejemplo_body={'path_inscripcion': 'inscripcion_buceo/'},
)
def registrar_inscripcion_buceo(conversacion, variables, config, endpoint=None) -> dict:
    ...
    if not cedula:
        return {'etiqueta': 'error', 'body': {}, 'status': 0,
                'error': 'Falta la cédula; no puedo registrar el cliente.'}

    edad = None
    try:
        edad = int(str(variables.get('edad')).strip())
    except (TypeError, ValueError):
        edad = None

    fnac = None
    fnac_raw = _txt('fecha_nacimiento')
    for fmt in ('%Y-%m-%d', '%d/%m/%Y', '%Y/%m/%d'):
        if fnac_raw:
            try:
                from datetime import datetime
                fnac = datetime.strptime(fnac_raw, fmt).date()
                break
            except ValueError:
                continue

    contacto = getattr(conversacion, 'contacto', None)
    telefono = getattr(contacto, 'contacto_numero', '') or ''
    sesion = getattr(contacto, 'sesion', None)
    departamento = None
    try:
        departamento = conversacion.estado_flujo.departamento
    except Exception:
        departamento = None
    ...
    try:
        ...
        return {'etiqueta': 'error', 'body': {}, 'status': 0,
                'error': f'No pude guardar el cliente: {ex.__class__.__name__}'}

    # Best-effort: replicar en SAGEST si el nodo tiene endpoint + path. Nunca
    # bloquea — el éxito ya está garantizado por el guardado local.
    sagest = {'intentado': False}
    path_insc = (config or {}).get('path_inscripcion') or ''
    body = (config or {}).get('body') or {}
    if endpoint and endpoint.base_url and path_insc and body:
        sagest['intentado'] = True
        try:
            url = endpoint.base_url.rstrip('/') + '/' + str(path_insc).lstrip('/')
            r = requests.post(
                url,
                headers=dict(endpoint.headers_default or {}) or {'Content-Type': 'application/json'},
                json=body,
                timeout=int((config or {}).get('timeout_seg') or endpoint.timeout_seg or 20),
            )
            sagest['status'] = r.status_code
            sagest['ok'] = 200 <= r.status_code < 300
        except Exception as ex:
            sagest['ok'] = False
            sagest['error'] = f'{ex.__class__.__name__}: {str(ex)[:150]}'
            logger.warning('registrar_inscripcion_buceo: SAGEST best-effort falló: %s', ex)
