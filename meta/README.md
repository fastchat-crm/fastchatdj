...
| `webhook.py` (nota 2026-07-16) | `validar_firma_hmac` acepta str o LISTA de secrets (`credenciales.py::get_meta_app_secrets` = `CredencialMetaApp.app_secret` + `CredencialMetaApp.app_secrets_extra` (uno por línea, UI /seguridad/credencial-meta/) + fallback `META_APP_SECRETS_EXTRA` de credenciales.json). Necesario cuando hay dos Meta Apps (una WhatsApp Cloud, otra Messenger/IG) suscritas: cada una firma con su propio app_secret y con uno solo los webhooks de la otra se rechazaban como "Firma HMAC inválida". El campo `app_secrets_extra` requiere migración (la genera/corre el developer). |
| `perfiles.py` | Verificadores de perfil por canal: pingean Graph con las credenciales de cada `Config*` y persisten los datos visibles (username / page_name / display_phone) + `ultima_sincronizacion`. Devuelven `(ok, info)` listos para JSON. También User Profile API de usuarios finales: `obtener_perfil_usuario_messenger(config_fb, psid)` (first_name/last_name/profile_pic — Messenger no expone username/email/teléfono) y `obtener_perfil_usuario_instagram(config_ig, igsid)` (name/username/profile_pic + follower_count y flags follow si la app tiene permiso; sin email/teléfono). Ambas devuelven `{'ok','error','nombre','username','foto','raw'}` con el error crudo de Graph cuando falla; Messenger tiene fallback `GET /{page_id}/conversations?user_id=<psid>` → `participants.name` (para apps sin Advanced Access donde el GET directo al PSID da 100.33). `diagnosticar_token(access_token)` hace GET /me y dice si el token vive y a qué página pertenece. Las consumen `whatsapp/meta_social_webhook_view.py::_enriquecer_perfil_social` y `whatsapp/funcionesWhatsappConversacion.py::consultar_datos_red`. |
...
| `instagram.py` | `InstagramService` y `MessengerService` — DMs vía Graph (`POST /{page_id}/messages`), y desde 2026-07: perfil (`obtener_perfil`), publicaciones (`listar_publicaciones`), comentarios (`responder_comentario`, `ocultar_comentario`) y private reply (`enviar_dm_desde_comentario`). Messenger hereda de Instagram cambiando el resolver de config y, desde 2026-07-14, overridea los endpoints que difieren para páginas FB: `obtener_perfil` (GET `/{page_id}`), `listar_publicaciones` (GET `/{page_id}/posts` normalizado al shape IG), `responder_comentario` (POST `/{comment_id}/comments`) y `ocultar_comentario` (`is_hidden`). `send_text_message` parte textos > 1000 chars en envíos secuenciales (límite de la Graph API para IG/Messenger). Desde 2026-07-15 todo el boilerplate HTTP (try/parse/shape del dict) vive en el helper `InstagramService._graph_request` — los métodos solo mapean campos; `send_media_message` ahora devuelve el shape estándar (`error` en vez del antiguo `body`) y `_ERROR_CONFIG` es atributo de clase (Messenger reporta `config_messenger_no_encontrada` también en envíos, antes heredaba el string de IG). La versión de Graph ya no está hardcodeada: `instagram.py` y `capi.py` importan `GRAPH_API_VERSION` de `meta/urls.py` (`settings.META_API_VERSION`). Comentarios IG con respuestas (fix 2026-07-28): `InstagramService.listar_comentarios_publicacion` pide `replies{...}` además de los campos base y aplana padre + respuestas en una sola lista (`parent_id` heredado del padre) — el edge `/{media_id}/comments` de IG solo trae primer nivel pero el `comments_count` del media cuenta las respuestas, así que el modal mostraba menos de lo que anunciaba la grilla. Equivale al `filter=stream` que Messenger ya usaba. La normalización vive en `_normalizar_comentario_ig()` y los campos en `_CAMPOS_COMENTARIO_IG`. Ojo con los scopes: leer comentarios de una **página de Facebook** exige `pages_read_user_content` (y `pages_manage_engagement` para responder/ocultar); `pages_read_engagement` solo cubre el contenido propio de la página y Graph responde `(#200) Missing Permissions`. Métricas de posts FB (fix 2026-07-20): `_CAMPOS_POSTS_BASE` pide `comments.filter(stream)` (el summary default solo cuenta comentarios de primer nivel, las respuestas no) y `reactions` en vez de `likes` (`likes.summary` solo cuenta el "Me gusta" clásico; ama/encanta/etc. no) — la grilla mostraba 0·1 cuando FB decía 2 likes·3 comentarios. |
