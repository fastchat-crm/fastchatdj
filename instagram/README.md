...
| `view_cuentas.py` | `/instagram/cuentas/` — conectar/editar/probar/suspender/eliminar cuentas. Acción `autodetectar`: con solo el Access Token extrae page_id / ig_user_id / @username. Al guardar prueba conexión y genera el webhook verify token. |
...
| `view_conversaciones.py` | `/instagram/conversaciones/` — listado de DMs del canal con filtros y deep-link `?conv=<encrypt_id>` al inbox compartido de `/whatsapp/conversaciones/`. |
| `view_comentarios.py` | `/instagram/comentarios/` — wrapper de `whatsapp.view_comentarios.comentariosView(canal_fijo='instagram')`. |
| `view_posts.py` | `/instagram/publicaciones/` — grilla en vivo de posts (`InstagramService.listar_publicaciones`) con likes/comentarios de IG + conteo de comentarios del CRM por `media_id`. |
