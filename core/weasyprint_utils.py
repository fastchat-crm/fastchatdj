...
class WeasyprintUtil:

    def __init__(self):
        raise Exception(f"noninstantiability")
    ...
    @staticmethod
    def django_file_fetcher(url, *args, **kwargs):
        if url.startswith('file://'):
            url = url[7:]

        if url.startswith('/static/'):
            # Convertir la URL de static a una ruta de sistema de archivos
            path = os.path.join(settings.STATIC_ROOT, url.replace('/static/', '', 1))
            with open(path, 'rb') as f:
                content = f.read()
            return dict(string=content, mime_type='application/octet-stream')

        if url.startswith('/media/'):
            # Convertir la URL de media a una ruta de sistema de archivos
            path = os.path.join(settings.MEDIA_ROOT, url.replace('/media/', '', 1))
            with open(path, 'rb') as f:
                content = f.read()
            return dict(string=content, mime_type='application/octet-stream')

        # Si no es una URL de static, media o file, se usa el fetcher por defecto
        ...
        if url.startswith('file:'):
            ...
            # path looks like a static file based on configured STATIC_URL
                ...
                if hasattr(staticfiles_storage, 'hashed_files'):
                    ...
                    relative_path = WeasyprintUtil.get_reversed_hashed_files()[relative_path]
