...
class VectorStoreManager:
    def __init__(self, storage_dir, provider: str, apikey: str, base_url: str = None):
        ...
        self.embeddings = self._provider_obj.get_embeddings(self.apikey, base_url=self.base_url)
    ...
    def _extract_raw_text(file_path: str) -> str:
        ...
        Delegado a la tubería única (Tika primero, loaders locales de respaldo).
        """
        return extraer_texto_archivo(file_path)
    ...
    def load_and_split(self, file_path, metadata=None):
        try:
            ...
            # Formatos sin loader local (docx, ppt, imágenes, ...) — vía Tika
            texto = extraer_texto_archivo(file_path)
            if not texto:
                raise ValueError("Formato de archivo no soportado o sin texto legible")
            return self.build_from_string(texto, metadata)

        if not any((d.page_content or '').strip() for d in docs):
            # PDF escaneado u otro archivo sin texto embebido — reintento vía Tika (OCR)
            texto = extraer_texto_archivo(file_path)
            if texto:
                return self.build_from_string(texto, metadata)
