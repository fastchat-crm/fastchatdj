...
| Asignar apenas llega la conversación | `whatsapp/procesar_mensaje.py` (rama `created`) con `SesionWhatsApp.auto_asignar_round_robin` | La conversación nace con asesor sin esperar el handoff. Silencioso a propósito: no pausa el bot, no avisa al cliente ni dispara el triple aviso (sería un aviso por lead). Cuando el flujo llega a "hablar con un asesor", `auto_asignar_agente(..., avisar_si_ya_asignado=True)` NO reasigna: pausa la IA, notifica al asesor que ya la tiene y manda el handoff al cliente |
| Dropdown manual | `whatsapp/forms.py:AsignarAgenteForm` | Mismo pool; muestra rol + carga |
| Asignación automática masiva | `whatsapp/funcionesWhatsappConversacion.py:asignacion_automatica_masiva_post` | Botón del inbox junto al chip "Sin asesor". Requiere el permiso por sesión `PerfilSesionWhatsApp.puede_asignar_masivo_asesores` (`permisos_sesion.puede_asignar_masivo`; el superusuario siempre puede). Se marca desde el modal "Usuarios asignables" de `/whatsapp/sesiones/` (acción `toggle_asignacion_masiva`). Itera las conversaciones abiertas sin asesor y llama `auto_asignar_agente(motivo='masivo', avisar_cliente=False, pausar_ia=False)` una por una: conserva `HistorialAsignacion` y el triple aviso al asesor, pero NO cierra la conversación, NO pausa el bot y NO le escribe al cliente. El reparto es justo porque `candidatos_ordenados` se recalcula en cada vuelta. Tope `LIMITE_ASIGNACION_MASIVA=200` |
...
| Archivo | Rol |
|---|---|
| `crm/helpers_asignacion.py` | Fuente única: `candidatos_ordenados`, `auto_asignar_agente`, `asesores_disponibles_sesion`, notificación. `notificar_agente_asignado` dispara TRIPLE aviso: Notificacion interna (+push), correo con link, y WhatsApp al teléfono del agente (`_avisar_agente_por_whatsapp`, usa la sesión de la conversación; normaliza 09XXXXXXXX → 593...; en Meta puede fallar fuera de la ventana 24h — best-effort logueado). **Cada intento (los 3 canales, éxito o fallo con detalle) se persiste en `crm.LogNotificacionAsignacion`** (`_log_notif`) — visible en el modal "Avisos al asesor" del inbox (action GET `logs-notificaciones`) y del kebab del tablero de sesiones (action `logs_notificaciones`). El WhatsApp al asesor incluye nombre del cliente + link `wa.me/<numero_cliente>` + link al panel. El kebab del tablero también tiene "Carga de asesores" (action `carga_asesores`): conversaciones abiertas por asesor, asignaciones 24h, disponibilidad y sin-asignar de la sesión |
